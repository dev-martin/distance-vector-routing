#ifndef _ES_C_
#define _ES_C_

/* $Id: es.c,v 1.1 2000/03/01 14:09:09 bobby Exp bobby $
 * ----
 * $Revision: 1.1 $
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
#include <sys/time.h>
#include <string.h>
#include <signal.h>     /* for signal() and SIGALRM */

#include "queue.h"
#include "common.h"
#include "es.h"
#include "ls.h"
#include "rt.h"
#include "n2h.h"

#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <netdb.h>

static struct el* g_el;
int time_between_global = 0;
int update_time_global = 0;
struct route_update {
	uint8_t type;
	uint8_t version;
	uint16_t num_updates;
	uint32_t update[256];
} __attribute__((packed));
void Alarm()     /* Handler for SIGALRM */
{	
	time_between_global = time_between_global +1;
	update_time_global = update_time_global + 1;
	alarm(1);
}
int init_new_el()
{
	InitDQ(g_el, struct el);
	assert (g_el);

	g_el->es_head = 0x0;
	return (g_el != 0x0);
}

void add_new_es()
{
	struct es* n_es;
	struct el* n_el = (struct el*)
		getmem (sizeof(struct el));

	struct el* tail = g_el->prev;
	InsertDQ(tail, n_el);

	// added new es to tail
	// lets start a new queue here
  
	{
		struct es* nhead = tail->es_head;
		InitDQ(nhead, struct es);

		tail = g_el->prev;     
		tail->es_head = nhead; 

		n_es = nhead;

		n_es->ev = _es_null;
		n_es->peer0 = n_es->peer1 = 
			n_es->port0 = n_es->port1 =
			n_es->cost = -1;
		n_es->name = 0x0;
	}
}

void add_to_last_es(e_type ev,
	node peer0, int port0,
	node peer1, int port1,
	int cost, char *name)
{
	struct el* tail = g_el->prev;
	bool local_event = false;
  
	assert (tail->es_head);

	// check for re-defined link (for establish)
	// check for local event (for tear-down, update)
	switch (ev) {
		case _es_link:
			// a local event?
			if ((peer0 == get_myid()) || peer1 == get_myid())
				local_event = true;
			break;
		case _ud_link:
			// a local event?
			if (geteventbylink(name))
				local_event = true;
			break;
		case _td_link:
			// a local event?
			if (geteventbylink(name))
				local_event = true;
			break;
		default:
			printf("[es]\t\tUnknown event!\n");
			break;
	}

	if (!local_event) {
		printf("[es]\t Not a local event, skip\n");
		return;
	}

	printf("[es]\t Adding into local event\n");

	{
		struct es* es_tail = (tail->es_head)->prev;
    
		struct es* n_es = (struct es*)
			getmem (sizeof(struct es));
    
		n_es->ev = ev;
		n_es->peer0 = peer0;
		n_es->port0 = port0;
		n_es->peer1 = peer1;
		n_es->port1 = port1;
		n_es->cost = cost;
		n_es->name = (char *)
			getmem(strlen(name)+1);
		strcpy (n_es->name, name);

		InsertDQ (es_tail, n_es);
	}
}

/*
 * A simple walk of event sets: dispatch and print a event SET every 2 sec
 */
void walk_el(int update_time, int time_between, int verb)
{	
	
	struct el *el;
	struct es *es_hd;
	struct es *es;
	
	
	struct sigaction myAction;       /* For setting signal handler */
	
	 /* Set signal handler for alarm signal */
    myAction.sa_handler = Alarm;
    if (sigfillset(&myAction.sa_mask) < 0) /* block everything in handler */
        DieWithSystemMessage("sigfillset() failed");
    myAction.sa_flags = 0;

    if (sigaction(SIGALRM, &myAction, 0) < 0)
    {
        DieWithSystemMessage("sigaction() failed for SIGALRM");
    }
  

  /* you will be using these varaibles, but for now ignoring them */
  
  UNUSED(verb);

	assert (g_el->next);
  
	print_el();

	/* initialize link set, routing table, and routing table */
	create_ls();
	create_rt();
	init_rt_from_n2h();
	
	el = g_el->next ;
	
	alarm(1);
	
	while( el != g_el ) {
		
		assert(el);
		es_hd = el->es_head;
		assert (es_hd);
		
		bool change = false;
	
	    while(time_between_global>time_between)
	    {   
			time_between_global=0;
			change = true;
		
			printf("[es] >>>>>>>>>> Dispatch next event set <<<<<<<<<<<<<\n");
			for (es=es_hd->next ; es!=es_hd ; es=es->next) {
				printf("[es] Dispatching next event ... \n");
				dispatch_event(es);
			}
			el = el->next;
		}
		/*
		 * --------------------------------------------------------- 
		 * Run DISTANCE VECTOR ALGORITHM here 
		 * --------------------------------------------------------- 
		 */
		 
		 //~ while(update_time_global>update_time)
		 //~ {	 
			//~ update_time_global=0;
			//~ change =true;
            //~ struct rte *rt;
			//~ struct rte *g_rt = ret_g_rte();
			//~ struct route_update rt_upd;
			
            //~ uint32_t dest;
            //~ uint32_t dest_cost;
            
            //~ int n_updates = 0;
			
			//~ rt_upd.type = htons(0x7);
			//~ rt_upd.version = htons(0x1);
			
			//~ for (rt = g_rt->next; rt != g_rt; rt = rt->next){
				//~ dest = rt->d;
				//~ dest_cost = rt->c;
				
				//~ rt_upd.num_updates = htons(n_updates+1);
				
				//~ rt_upd.update[n_updates] = htonl((dest<<24)|(dest_cost));
				
				//~ n_updates++;
			//~ }
			 
			//~ struct link *ls;
			//~ struct link *g_ls = get_ls();
			
			
			
			//~ for (ls = g_ls->next; ls != g_ls; ls= ls->next){
				//~ struct hostent *h;
				//~ if(ls->peer0 == get_myid())
				//~ {   
					//~ h= gethostbyname(gethostbynode(get_myid()));
					
					//~ if (sendto(ls->sockfd0, &rt_upd, sizeof(rt_upd), 0, (struct sockaddr *)   h->h_addr_list[0], sizeof(h->h_addr_list[0])) != sizeof(rt_upd))
							//~ DieWithSystemMessage("sendto() failed");
				//~ }
				//~ else
				//~ {	
					//~ h = gethostbyname(gethostbynode(get_myid()));
					//~ if (sendto(ls->sockfd0, &rt_upd, sizeof(rt_upd), 0, (struct sockaddr *)   h->h_addr_list[0] , sizeof(h->h_addr_list[0])) != sizeof(rt_upd))
							//~ DieWithSystemMessage("sendto() failed"); 
				//~ }
			//~ }
				
			
			//~ poll
			//~ recvfrom
		 //~ }
		 
		 
        if(change == true)
        {	
			printf("[es] >>>>>>> Start dumping data stuctures <<<<<<<<<<<\n");
			print_n2h();
			print_ls();
			print_rt();
		}
	}
}

/*
 * -------------------------------------
 * Dispatch one event
 * -------------------------------------
 */
void dispatch_event(struct es* es)
{	
	assert(es);
	struct link *i;
	switch (es->ev) {
		case _es_link:
			add_link(es->peer0, es->port0, es->peer1, es->port1,
				es->cost, es->name);
			if ((unsigned int)es->peer0 ==  get_myid())
			{   
				struct rte *i;
				if((i = find_rte(es->peer1))!=NULL)
				{
			     	if(update_rte(es->peer1,es->cost,es->peer0)<0)
							DieWithSystemMessage("update_rte() failed");
				}
				else
				{
					if(add_rte(es->peer1,es->cost,es->peer0)<0)
						DieWithSystemMessage("add_rte() failed");
				}
			}
				
			else
			{   
				struct rte *i;
				if((i = find_rte(es->peer0))!=NULL)
				{
					if(i->c > (unsigned int)es->cost)
					{
						if(update_rte(es->peer0,es->cost,es->peer1)<0)
							DieWithSystemMessage("update_rte() failed");
					}
				}
				else
				{
					if(add_rte(es->peer0,es->cost,es->peer1)<0)
						DieWithSystemMessage("add_rte() failed");
				}
			}
			break;
		case _ud_link:
			
			i=find_link(es->name);
			
			if (i->peer0 == get_myid())
			{   
				if(update_rte(i->peer1,es->cost,i->peer0)<0)
					DieWithSystemMessage("update_rte() failed");
			}
				
			else
			{   
				if(update_rte(i->peer0,es->cost,i->peer1)<0)
					DieWithSystemMessage("update_rte() failed");
			}
			
			ud_link(es->name, es->cost);
			break;
		case _td_link:
			
			i=find_link(es->name);
			
			if(i->peer0 == get_myid())
			{
				if(update_rte(i->peer1,-1,i->peer0)<0)
						DieWithSystemMessage("del_rte() failed");
			}
			else
			{
				if(update_rte(i->peer0,-1,i->peer1)<0)
						DieWithSystemMessage("del_rte() failed");
			}
			del_link(es->name);
			
			break;
		default:
			printf("[es]\t\tUnknown event!\n");
			break;
	}

}

/*
 * print out the whole event LIST
 */
void print_el()
{
	struct el *el;
	struct es *es_hd;
	struct es *es;

	assert (g_el->next);

	printf("\n\n");
	printf("[es] >>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<\n");
	printf("[es] >>>>>>>>>> Dumping all event sets  <<<<<<<<<<<<<\n");
	printf("[es] >>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<\n");

	for (el = g_el->next ; el != g_el ; el = el->next) {

		assert(el);
		es_hd = el->es_head;
		assert (es_hd);
	
		printf("\n[es] ***** Dumping next event set *****\n");

		for (es=es_hd->next ; es!=es_hd ; es=es->next)
			print_event(es);
	}     
}

/*
 * print out one event: establish, update, or, teardown
 */
void print_event(struct es* es)
{
	assert(es);

	switch (es->ev) {
		case _es_null:
			printf("[es]\t----- NULL event -----\n");
			break;
		case _es_link:
			printf("[es]\t----- Establish event -----\n");
			break;
		case _ud_link:
			printf("[es]\t----- Update event -----\n");
			break;
		case _td_link:
			printf("[es]\t----- Teardown event -----\n");
			break;
		default:
			printf("[es]\t----- Unknown event-----\n");
			break;
	}
	printf("[es]\t link-name(%s)\n",es->name);
	printf("[es]\t node(%d)port(%d) <--> node(%d)port(%d)\n", 
		es->peer0,es->port0,es->peer1,es->port1);
	printf("[es]\t cost(%d)\n", es->cost);
}

struct es *geteventbylink(char *lname)
{
	struct el *el;
	struct es *es_hd;
	struct es *es;

	assert (g_el->next);
	assert (lname);

	for (el = g_el->next ; el != g_el ; el = el->next) {

		assert(el);
		es_hd = el->es_head;
		assert (es_hd);
	
		for (es=es_hd->next ; es!=es_hd ; es=es->next)
			if (!strcmp(lname, es->name))
				return es;
	}
	return 0x0;
}

#endif

